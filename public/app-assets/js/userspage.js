        ...
        onValue(users, (snapshot) => {
            ...
            if (snapshot) {
                snapshot.forEach(function (doc) {
                    ...
                    var address = '<a style="color: #fff;cursor:pointer;" onclick="showAddressModal(\'' + doc.key + '\')" class="btn btn-primary badge-shadow"><i class="fas fa-eye"></i></a>' + input;
                    
                    if(status == "undefined" || status == 1){
                        var label = '<div class="custombadge-outline col-green custombadge-shadow">Active</div>';
                        var action = '<a style="color: #fff;cursor:pointer;" onclick="showBlockModal(\'' + doc.key + '\')" class="btn btn-danger badge-shadow"><i class="fas fa-ban"></i></a><a style="color: #fff;cursor:pointer;margin-left:2px;" onclick="showDeleteModal(\'' + doc.key + '\')" class="btn btn-danger badge-shadow"><i class="fas fa-trash"></i></a>';
                        var rowId = "1";
                    }
                    else{
                        var label = '<div class="custombadge-outline col-red custombadge-shadow">Blocked</div>';
                        var action = '<a style="color: #fff;cursor:pointer;" onclick="showUnBlockModal(\'' + doc.key + '\')" class="btn btn-success badge-shadow"><i class="fas fa-check-circle"></i></a><a style="color: #fff;cursor:pointer;margin-left:2px;" onclick="showDeleteModal(\'' + doc.key + '\')" class="btn btn-danger badge-shadow"><i class="fas fa-trash"></i></a>';
                        var rowId = "2";
                    }
